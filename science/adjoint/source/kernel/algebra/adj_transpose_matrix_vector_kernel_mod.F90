...
!-----------------------------------------------------------------------------
!> @brief Adjoint of transpose_matrix_vector_kernel
module adj_transpose_matrix_vector_kernel_mod
  ...
  interface adj_transpose_matrix_vector_code
    module procedure  &
      adj_transpose_matrix_vector_code_r_single, &
      adj_transpose_matrix_vector_code_r_double
  end interface

  contains

  !> @brief Computes adjoint of lhs = matrix^T*x where matrix^T is the transpose of the matrix
  !! @param[in]      cell      Horizontal cell index
  !! @param[in]      nlayers   Number of layers
  !! @param[in]      lhs       Left hand side field of lhs = matrix^T*x
  !! @param[in,out]  x         Right hand side field of lhs = matrix^T*x
  ...
  ! R_SINGLE PRECISION
  ! ==================
  subroutine adj_transpose_matrix_vector_code_r_single(cell,              &
                                                       nlayers,           &
                                                       lhs, x,            &
                                                       ncell_3d,          &
                                                       matrix,            &
  ...
  ! R_DOUBLE PRECISION
  ! ==================
  subroutine adj_transpose_matrix_vector_code_r_double(cell,              &
                                                       nlayers,           &
                                                       lhs, x,            &
                                                       ncell_3d,          &
                                                       matrix,            &
