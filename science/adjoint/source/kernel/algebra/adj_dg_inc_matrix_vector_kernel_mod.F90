...
!-----------------------------------------------------------------------------
!> @brief Adjoint of dg_inc_matrix_vector_kernel, for use on discontinous spaces
module adj_dg_inc_matrix_vector_kernel_mod
  ...
  type(arg_type) :: meta_args(3) = (/                                      &
       arg_type(GH_FIELD,    GH_REAL, GH_READ, ANY_DISCONTINUOUS_SPACE_1), & ! lhs
       arg_type(GH_FIELD,    GH_REAL, GH_INC,  ANY_SPACE_1),               & ! x
       arg_type(GH_OPERATOR, GH_REAL, GH_READ, ANY_DISCONTINUOUS_SPACE_1,  & ! matrix
                                               ANY_SPACE_1)                &
       /)
  ...
  interface adj_dg_inc_matrix_vector_code
    module procedure  &
      adj_dg_inc_matrix_vector_code_r_single, &
      adj_dg_inc_matrix_vector_code_r_double
  end interface adj_dg_inc_matrix_vector_code

  contains

  !> @brief Computes adjoint of lhs = lhs + matrix*x for discontinuous function spaces
  !!        real32 and real64 variants
  !> @param[in]     cell      Horizontal cell index
  !> @param[in]     nlayers   Number of layers
  !> @param[inout]  lhs       Left hand side field of lhs = matrix*x
  !> @param[in]     x         Right hand side field of lhs = matrix*x
  ...
  ! R_SINGLE PRECISION
  ! ==================
  subroutine adj_dg_inc_matrix_vector_code_r_single(cell,              &
                                                    nlayers,           &
                                                    lhs, x,            &
                                                    ncell_3d,          &
                                                    matrix,            &
  ...
  ! R_DOUBLE PRECISION
  ! ==================
  subroutine adj_dg_inc_matrix_vector_code_r_double(cell,              &
                                                    nlayers,           &
                                                    lhs, x,            &
                                                    ncell_3d,          &
                                                    matrix,            &
