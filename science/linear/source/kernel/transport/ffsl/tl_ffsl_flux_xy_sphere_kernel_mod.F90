...
!> @brief Calculates the horizontal mass flux for FFSL for the linear model, with
!!        a consideration of the swapping of directions at panel edges on the cubed
!!        sphere.
...
module tl_ffsl_flux_xy_sphere_kernel_mod
  ...
  type, public, extends(kernel_type) :: tl_ffsl_flux_xy_sphere_kernel_type
    private
  ...
  contains
    procedure, nopass :: tl_ffsl_flux_xy_sphere_code
  end type

  !-----------------------------------------------------------------------------
  ! Contained functions/subroutines
  !-----------------------------------------------------------------------------
  public  :: tl_ffsl_flux_xy_sphere_code
  private :: tl_ffsl_flux_xy_sphere_1d
contains

  !> @brief Compute the horizontal fluxes for FFSL for the tangent linear model on the sphere.
  ...
  subroutine tl_ffsl_flux_xy_sphere_code( nlayers,             &
                                          flux_pert,           &
                                          flux_ls,             &
                                          field_for_x,         &
                                          stencil_sizes_x,     &
    ...
    use tl_ffsl_flux_xy_kernel_mod, only: tl_ffsl_flux_xy_1d
    ...
    if (use_spherical_treatment) then
      call tl_ffsl_flux_xy_sphere_1d( nlayers,                &
                                      .true.,                 &
                                      flux_pert,              &
                                      flux_ls,                &
                                      field_for_x,            &
  ...
  end subroutine tl_ffsl_flux_xy_sphere_code
...
! ============================================================================ !
  ...
  subroutine tl_ffsl_flux_xy_sphere_1d( nlayers,                 &
                                        x_direction,             &
                                        flux_pert,               &
                                        flux_ls,                 &
                                        field,                   &
    ...
    real(kind=r_tran), target, intent(in) :: ls_field(undf_w3)
    real(kind=r_tran), target, intent(in) :: ls_field_swapped(undf_w3)
    ...
    do df_idx = 1, ABS(face_selector(map_w3_2d(1)))
      ...
      do j = 1, recon_size
        ...
        do k = 1, nlayers
          ...
          ls_field_local(k, loc_idx(k)) = (                                    &
            (1.0_r_tran - swap_switch(k))*ls_field(col_idx+k-1)                &
            + swap_switch(k)*ls_field_swapped(col_idx+k-1)                     &
          )
        end do
      end do
...
end module tl_ffsl_flux_xy_sphere_kernel_mod
